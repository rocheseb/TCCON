...
for spectrum in select_spectra:
	...
	for j in range(len(species)-3):
		...
		fig.add_tools( HoverTool(mode='vline',line_policy='nearest',renderers=[plots[j]],names=[header[j+3]],tooltips=OrderedDict( [('name',header[j+3]),('(x;y)','(@x{0.00} ; @y{0.000})')] ), callback=CustomJS(args=dict(cb=checkbox),code=hover_code)) )
	...
	fig.add_tools( HoverTool(mode='vline',line_policy='nearest',renderers=[plots[j+1]],names=['Tm'],tooltips=OrderedDict( [('name','Measured'),('(x;y)','(@x{0.00} ; @y{0.000})')] ), callback=CustomJS(args=dict(cb=checkbox),code=hover_code)) )
	...
	fig.add_tools( HoverTool(mode='vline',line_policy='nearest',renderers=[plots[j+2]],names=['Tc'],tooltips=OrderedDict( [('name','Calculated'),('(x;y)','(@x{0.00} ; @y{0.000})')] ), callback=CustomJS(args=dict(cb=checkbox),code=hover_code)) )
	...
	fig_resid.add_tools(HoverTool(mode='vline',line_policy='nearest',names=['residuals'],tooltips={'(x;y)':'($~x{0.00} ; @y{0.000})'}))
